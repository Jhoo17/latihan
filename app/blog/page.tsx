...
export default async function BlogPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">GeoConsult Blog & Publications</h1>
          ...
          <Card key={post.sys.id}>
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <Badge variant={post.fields.type === 'publication' ? 'default' : 'secondary'}>
                  {post.fields.type}
                </Badge>
              </div>
              <p className="text-sm text-gray-600 mb-2">Published on: {new Date(post.fields.date).toLocaleDateString()}</p>
